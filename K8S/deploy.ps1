...
function Wait-ForPodsReady {
    Write-Host "Comprobando el estado de los pods en el namespace urban..."
    while ($true) {
        # Obtener el estado de los pods en el namespace urban
        $podStatus = kubectl get pods -n urban --no-headers

        # Contar el número de pods en estado Running/Completed
        $numReady = ($podStatus -match 'Running|Completed').Count

        # Obtener el número total de pods esperados
        $numExpected = $podStatus.Count

        # Si todos los pods están listos, salir del bucle
        if ($numReady -eq $numExpected) {
            Write-Host "Todos los pods en el namespace urban están listos."
            break
        }
...
Write-Host "Desplegando el controlador de Ingress..."
...
Write-Host "Desplegando la configuración de Ingress..."
kubectl apply -f ./deployments/ingress/urban-ingress.yaml

# Desplegar Cronjobs
Write-Host "Desplegando Cronjobs..."
kubectl apply -f ./cronjobs

# Desplegar el dashboard
Write-Host "Desplegando el dashboard..."
